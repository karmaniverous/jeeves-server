...
function initPanzoomOnContainer(
  ...
  const enterFullscreen = () => {
    ...
    pz.destroy();
    // Use double-rAF + setTimeout to ensure layout is fully settled
    requestAnimationFrame(() => { requestAnimationFrame(() => { setTimeout(() => {
      ...
      if (s && viewport.clientWidth && viewport.clientHeight) {
        ...
        // Scale SVG to fit viewport (the SVG's viewBox handles the internal scaling)
        const fitScale = Math.min(vw / svgW, vh / svgH, 1);
        const fittedW = svgW * fitScale;
        const fittedH = svgH * fitScale;
        // Set SVG to the fitted size
        ...
        // Center the inner div containing the SVG
  ...
  const exitFullscreen = () => {
    isFullscreen = false;
    // Restore SVG to responsive sizing
    const s = inner.querySelector('svg');
    if (s) {
