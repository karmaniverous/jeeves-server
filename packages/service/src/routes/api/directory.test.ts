...
/**
 * Test the directory entry mapping logic used by the directory route.
 * Extracted from the route handler to test independently.
 */
...
describe('directory entry mapping', () => {
  ...
  it('returns itemCount for directory entries', () => {
  ...
  it('returns null itemCount when directory read fails', () => {
    // Map a non-existent directory to simulate permission denied
    const fakeDirent = {
      name: 'nonexistent',
      isDirectory: () => true,
      isFile: () => false,
    ...
    const mapped = mapDirectoryEntry(fakeDirent, tmpDir);
    // statSync throws for nonexistent → mtime and itemCount are null
    expect(mapped.itemCount).toBeNull();
